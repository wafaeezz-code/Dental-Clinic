...
export function FAQ() {
  ...
  return (
    <section className="bg-white py-[80px]" style={{ fontFamily: "'Manrope', sans-serif" }}>
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 md:px-8 lg:px-[80px] py-8 sm:py-12 md:py-16 lg:py-[80px]">
          <div className="flex flex-col lg:flex-row gap-6 md:gap-8 lg:gap-[80px]">
            <div className="shrink-0 pt-0 lg:pt-[8px]">
            <div className="bg-[#e1f8fa] rounded-[20px] px-[16px] py-[6px] inline-block mb-[12px]">
                <span className="text-[#14bec9] text-sm md:text-base lg:text-[16px]" style={{ fontWeight: 200 }}>Questions fréquentes</span>
            </div>
              <h2 className="text-[#173435] text-xl sm:text-2xl md:text-3xl lg:text-[42px] tracking-[-1px] leading-[1.25] lg:leading-[52.5px] w-auto lg:w-[300px]" style={{ fontWeight: 400 }}>
              Foire aux questions
            </h2>
          </div>
          ...
          <div className="flex-1">
            {faqItems.map((item, index) => (
              <div key={index} className="border-b border-[#d8dede]">
                <button
                  className="w-full flex items-start lg:items-center justify-between py-3 md:py-4 lg:py-[19px] text-left gap-4 md:gap-5 lg:gap-[20px]"
                  onClick={() => setOpenIndex(openIndex === index ? null : index)}
                >
                  <span className="text-[#173435] text-base sm:text-lg md:text-xl lg:text-[21px]" style={{ fontWeight: 200 }}>
                    {item.question}
                  </span>
                  {openIndex === index ? <MinusIcon /> : <PlusIcon />}
                </button>
                {openIndex === index && (
                  <div className="pb-4 md:pb-5 lg:pb-[21px] text-[#173435] text-sm md:text-base lg:text-[16px] leading-[1.5] lg:leading-[21px]" style={{ fontWeight: 200 }}>
